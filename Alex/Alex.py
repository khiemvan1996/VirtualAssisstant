...
def open_application(text):
    if "google" in text:
        ...
        time.sleep(2)
        os.startfile('C:\Program Files\Microsoft Office\Office16\WINWORD.EXE') # Ở đây cũng như ở trên
    elif "excel" in text:
        speak("Mở Microsoft Excel")
        time.sleep(2)
        os.startfile('C:\Program Files\Microsoft Office\Office16\EXCEL.EXE') # Ở trển cũng giống dưới này =))
    else:
...
def change_wallpaper():
    ...
    url = 'https://api.unsplash.com/photos/random?client_id=' +1\
...
def assistant():
    ...
    if name:
        ...
        time.sleep(3)
        speak("Chào Anh yêu , người yêu ảo có thể giúp gì được cho anh ạ")
        time.sleep(5)
        while True:
            text = get_text()
            if not text:
                break
            elif "dừng" in text or "không cần" in text or "tạm biệt" in text or "chào robot" in text or "ngủ thôi" in text:
                stop()
                break
